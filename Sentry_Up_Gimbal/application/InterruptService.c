...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
    ...
    {
		case CAN_RC_CONTROL_ID:
        {
			CAN_RC_PACK((CAN_RC_DATA_t *)rx_data);
            break;
        }
		...
		case CHASSIS_MODE_ID:
		{
			Chassis_Mode = rx_data[0];
		}
		case REFEREE_DATA_ID:
		{
			Get_Referee_Data(rx_data);
		}
        default:
        {
            break;
...
{
	CanSendMessage(&COMMUNICATE_CANPORT, 0x129, 6, (uint8_t *)&RefereeInformation.RobotState);
}
