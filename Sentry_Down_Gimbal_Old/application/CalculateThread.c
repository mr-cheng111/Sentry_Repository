...
void GimbalFireModeUpdate(void)
{
	...
	else if(Remote.rc.s[0] != RC_SW_MID
	        ||Remote.rc.s[1] != RC_SW_DOWN)
	{
		Ammo_Counter = 0;
		Ammo_Flag = 0;
	}
	if ( Ammo_Flag && Gimbal.StateMachine != GM_MATCH )
	{
		...
		{
			if(Remote.rc.ch[4] == 660 && Gimbal.StateMachine == GM_TEST)						
			{
...
//				&&(Gimbal.Referee.CurrentHeat < MAX_HEAT)
				&&(Game_State.Game_Start_State == Game_START)
				&&(Gimbal.Chassis_Mode == CHASSIS_FAST))//判断是否进入比赛模式，在未进入比赛模式前，将无法打子弹,不打工程
			{
				Gimbal.FireMode = GM_FIRE_BUSY;
				//此处需要加上射速设置
...
void RefereeHeat_init(void)
{
	Gimbal.Referee.CoolingHeat = Referee.Realtime.Ammo0Heat;
	Gimbal.Referee.CoolingHeat = Referee.Ammo0Limit.Cooling;
	Gimbal.Referee.MaxHeat = Referee.Ammo0Limit.Heat;
